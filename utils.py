...
def apply_kernel_at(get_value, kernel, i, j):
    """ Применяет маску к текущей части изображения, с координатами i,j """
...
def apply_to_each_pixel(pixels, f):
    """ Выполняет функцию f для каждого пикселя изображения """
...
def flatten(ls):
    """ Склеивает списки, находящиеся в ls """
    result = []
    for i in ls:
        result += i
    return result
...
def gauss(x, y):
    ssigma = 1.0
    temp = (1 / (2 * math.pi * ssigma)) * math.exp(-(x * x + y * y) / (2 * ssigma))
    return temp
...
def kernel_from_function(size, f):
    ...
    for i in range(0, size):
        for j in range(0, size):
            temp = f(i - size // 2, j - size // 2)
            kernel[i].append(temp)
    return kernel
...
def apply_kernel(pixels, kernel):
    """ Применяет маску ко всему изображению изображению. """
    apply_kernel_with_f(pixels, kernel, lambda old, new: new)


def apply_kernel_with_f(pixels, kernel, f):
    """ Применяет маску ко всему изображению изображению.
    Для каждого участка выполняет функцию f(old_pixel,new_pixel)"""
    size = len(kernel)
...
def load_image(im):
    """ Звгружает PIL-изображение в массив """
    (x, y) = im.size
    im_load = im.load()
...
def load_pixels(im, pixels):
    """ Звгружает массив в PIL-изображение """
    (x, y) = im.size
    im_load = im.load()
